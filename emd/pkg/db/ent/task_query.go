...
func (tq *TaskQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Task, error) {
	...
	if query := tq.withUsers; query != nil {
		if err := tq.loadUsers(ctx, query, nodes,
			func(n *Task) { n.Edges.Users = []*User{} },
			func(n *Task, e *User) { n.Edges.Users = append(n.Edges.Users, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TaskQuery) loadUsers(ctx context.Context, query *UserQuery, nodes []*Task, init func(*Task), assign func(*Task, *User)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(task.UsersTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(task.UsersPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(task.UsersPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(task.UsersPrimaryKey[1]))
